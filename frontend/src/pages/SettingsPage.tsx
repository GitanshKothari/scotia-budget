...
const SettingsPage: React.FC = () => {
  ...
  const [formData, setFormData] = useState({ name: '', preferredCurrency: 'CAD' });
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    if (user) {
      setFormData({ name: user.name, preferredCurrency: user.preferredCurrency });
    }
    fetchCategories();
  }, [user]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow">
        ...
        <div>
          <h4 className="text-lg font-medium text-gray-900 dark:text-white mb-2">Custom Categories</h4>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
            {customCategories.map((cat) => (
              <div key={cat.id} className="p-2 bg-blue-100 dark:bg-blue-900 rounded text-sm text-gray-700 dark:text-gray-300 flex justify-between">
                <span>{cat.name}</span>
                <button
                  onClick={async () => {
